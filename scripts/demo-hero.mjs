 ...
 *
 * Writes: examples/demo-last-run.json
 */
...
async function main() {
  ...
  log.push(step('navigate example.com', r.status === 200 || r.status === 422, r.data.summary || r.data.error))
  ...
  const outPath = join(root, 'examples', 'demo-last-run.json')
